...
function canRespawn(p) {
  return p && p.chances <= p.revealed && !p.respawnUsed &&
    game.cards.filter(c => !c.revealed).length > 2 && game.started;
}
...
function respawnPlayer(p) {
  ...
  const activeNames = new Set(
    game.cards.filter(c => !c.revealed).map(c => c.imgName)
  );
  ...
  if (!rebuildUnrevealedAssignments()) return false;
  broadcastState();
  return true;
}
...
function isRoseGift(data) {
  const name = String(
    data?.giftDetails?.giftName ?? data?.extendedGiftInfo?.name ??
    data?.giftName ?? data?.gift?.name ?? ''
  ).toLowerCase();
  const describe = String(
    data?.giftDetails?.giftDescribe ?? data?.extendedGiftInfo?.describe ??
    data?.describe ?? data?.gift?.describe ?? ''
  ).toLowerCase();
  return name.includes('rose') || name.includes('وردة') ||
...
async function connectTikTok(username) {
  ...
  // This is intentionally the same connector setup and event flow as the
  // working project supplied by the user. No custom SignConfig/Tik.Tools is used.
  ...
  connection.on(WebcastEvent.GIFT, (data) => {
    if (generation !== connectionGeneration) return;
    const p = playerFromEvent(data);
    const player = findPlayer(p.uid);
    const repeatEnd = data?.repeatEnd ?? data?.repeat_end ?? true;
    if (player && isRoseGift(data) &&
        (repeatEnd === true || repeatEnd === 1 || repeatEnd === '1')) {
