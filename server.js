...
function forceCloseModal() { if(game.modal){ game.modal=null; broadcastState(); } }
...
function connectTikTok(username) {
  ...
  connection.on(WebcastEvent.CHAT, data => {
    sendChatLine(data);
    const uid=clean(data.user?.uniqueId);
    const p=findPlayer(uid);
    const comment=String(data.comment||'').trim();
    if(!game.started){
      const pass=String(live?.password || 'تم');
      if(comment === pass && !p && game.players.size < 30){
        game.players.set(uid,{id:uid,uniqueId:uid,nickname:data.user?.nickname||uid,profilePictureUrl:data.user?.profilePictureUrl||'',chances:game.multiplier,revealed:0,respawnUsed:false});
        broadcastState();
      }
      return;
    }
  ...
  connection.on(WebcastEvent.GIFT, data => {
    const p=findPlayer(data.user?.uniqueId);
    if(p && isRoseGift(data) && data.repeatEnd !== false) respawnPlayer(p);
  });
